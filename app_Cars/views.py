...
def cars_view(request):
    ...
    if form.is_valid():
        ...
        if form.cleaned_data["search"] and form.cleaned_data["search_year"]:
            cars = Car.objects.filter(Q(manufacturer__contains=form.cleaned_data["search"]) | Q(
                modelCar__contains=form.cleaned_data["search"]))
            cars =cars.filter(year_manufacture__gt=form.cleaned_data["search_year"])


        if form.cleaned_data["search_year"]:
            cars=Car.objects.filter(year_manufacture__gt=form.cleaned_data["search_year"])
        # if form.cleaned_data["search_transmission"]:
        #
        #     cars = Car.objects.filter(transmission__contains=form.cleaned_data["search_transmission"])
        # else:
        #      cars = Car.objects.all()
